	...
	class BasicGoogleMapsPlacemarks {
		...
		public function init() {
			...
			$this->options = array_merge(
				$defaultOptions,
				get_option( self::PREFIX . 'options', array() )
			);
			...
			$this->userMessageCount       = array(
				'updates' => count( $this->options['updates'] ),
				'errors'  => count( $this->options['errors'] )
			);
		...
		protected function singleActivate() {
			...
			if ( ! get_option( self::PREFIX . 'map-address' ) ) {
				add_option( self::PREFIX . 'map-address', __( 'Seattle', 'basic-google-maps-placemarks' ) );
			}
		...
		public function addFeaturedImageSupport() {
			...
			if ( version_compare( $wp_version, '3.3', "<=" ) && is_admin() && function_exists( 'is_multisite' ) && is_multisite() ) {
				...
				if ( ! array_key_exists( 'image', $mediaButtons ) || ! $mediaButtons['image'] ) {
					$this->enqueueMessage( sprintf(
						__( "%s requires the Images media button setting to be enabled in order to use custom icons on markers, but it's currently turned off. If you'd like to use custom icons you can enable it on the <a href=\"%s\">Network Settings</a> page, in the Upload Settings section.", 'basic-google-maps-placemarks' ),
						BGMP_NAME,
		...
		protected function mapShortcodeCalled() {
	        ...
	        // note: this needs to run after the above code, so that templates can call do_shortcode(...) from templates that don't have $post, like 404.php. See link in phpDoc @link for background.
			if ( ! $post ) {
				return false;
			}
		...
		/**
		 * Get the user's personalized URL for the Maps API
		 *
		 * @todo maybe provide a smooth oAuth flow for obtaining an API key, rather than creating an "app" in dev console
		 * @todo add `async` and `defer` to script tag. Core currently supports that via `script_loader_tag`
		 *       (see r30403), but that's ugly. Probably wait for #22249 instead.
		 * @todo see if can detect if map fails to load, and then give instructions for getting api key
		 *
		 * @return string
		 */
		protected function getGoogleMapsApiUrl() {
			...
			$urlParameters = array();
			$language      = apply_filters( 'bgmp_map-language', '' );

			if ( ! empty ( $language ) ) {
				$this->deprecated( 'bgmp_map-language', 'bgmp_maps-api-url-parameters', '1.10.7' );
				$urlParameters['language'] = rawurlencode( $language );
			}
			...
			$urlParameters = apply_filters( 'bgmp_maps-api-url-parameters', $urlParameters );
		...
		public function createPostType() {
			...
			if ( ! post_type_exists( self::POST_TYPE ) ) {
				$labels = array(
					'name'               => __( 'Placemarks',                   'basic-google-maps-placemarks' ),
					'singular_name'      => __( 'Placemark',                    'basic-google-maps-placemarks' ),
					'add_new'            => __( 'Add New',                      'basic-google-maps-placemarks' ),
					'add_new_item'       => __( 'Add New Placemark',            'basic-google-maps-placemarks' ),
					'edit'               => __( 'Edit',                         'basic-google-maps-placemarks' ),
					'edit_item'          => __( 'Edit Placemark',               'basic-google-maps-placemarks' ),
					'new_item'           => __( 'New Placemark',                'basic-google-maps-placemarks' ),
					'view'               => __( 'View Placemark',               'basic-google-maps-placemarks' ),
					'view_item'          => __( 'View Placemark',               'basic-google-maps-placemarks' ),
					'search_items'       => __( 'Search Placemarks',            'basic-google-maps-placemarks' ),
					'not_found'          => __( 'No Placemarks found',          'basic-google-maps-placemarks' ),
					'not_found_in_trash' => __( 'No Placemarks found in Trash', 'basic-google-maps-placemarks' ),
					'parent'             => __( 'Parent Placemark',             'basic-google-maps-placemarks' ),
				);

				$postTypeParams = array(
					'labels'          => $labels,
					'singular_label'  => __( 'Placemarks', 'basic-google-maps-placemarks' ),
					'public'          => true,
					'menu_position'   => 20,
					'hierarchical'    => false,
		...
		public function addMetaBoxes() {
			...
			add_meta_box(
				...
				__( 'Placemark Address', 'basic-google-maps-placemarks' ),
			...
			add_meta_box(
				...
				__( 'Stacking Order', 'basic-google-maps-placemarks' ),
		...
		public function saveCustomFields( $postID ) {
			...
			if ( isset( $_POST[ self::PREFIX . 'zIndex' ] ) ) {
				if ( filter_var( $_POST[ self::PREFIX . 'zIndex' ], FILTER_VALIDATE_INT ) === false ) {
					...
					$this->enqueueMessage( __( 'The stacking order has to be an integer.', 'basic-google-maps-placemarks' ), 'error' );
				} else {
		...
		public function geocode( $address ) {
			...
			$geocodeResponse = wp_remote_get( $this->getGoogleGeocodingApiUrl( $address ) );
			// @todo - esc_url() on address?

			if ( is_wp_error( $geocodeResponse ) ) {
				$this->enqueueMessage( sprintf(
					__( '%s geocode error: %s', 'basic-google-maps-placemarks' ),
					BGMP_NAME,
			...
			if ( ! isset( $geocodeResponse['response']['code'] ) || ! isset( $geocodeResponse['response']['message'] ) ) {
				$this->enqueueMessage( sprintf(
					__( '%s geocode error: Response code not present', 'basic-google-maps-placemarks' ),
					BGMP_NAME
				), 'error' );
				...
				*/
				...
				$this->enqueueMessage( sprintf(
					__( '<p>%s geocode error: %d %s</p> <p>Response: %s</p>', 'basic-google-maps-placemarks' ),
					BGMP_NAME,
			...
			if ( function_exists( 'json_last_error' ) && json_last_error() != JSON_ERROR_NONE ) {
				...
				$this->enqueueMessage( sprintf( __( '%s geocode error: Response was not formatted in JSON.', 'basic-google-maps-placemarks' ), BGMP_NAME ), 'error' );

				return false;
			}

			if ( isset( $coordinates->status ) && $coordinates->status == 'REQUEST_DENIED' ) {
				$this->enqueueMessage( sprintf( __( '%s geocode error: Request Denied.', 'basic-google-maps-placemarks' ), BGMP_NAME ), 'error' );

				return false;
			}

			if ( ! isset( $coordinates->results ) || empty( $coordinates->results ) ) {
				$this->enqueueMessage( __( "That address couldn't be geocoded, please make sure that it's correct.", 'basic-google-maps-placemarks' ), "error" );
				$this->enqueueMessage(
					__( "Geocode response:", 'basic-google-maps-placemarks' ) . ' <pre>' . print_r( $coordinates, true ) . '</pre>',
					"error"
				);
		...
		/**
		 * Get the URL for Geocoding API requests
		 *
		 * @param string $address
		 *
		 * @return string
		 */
		protected function getGoogleGeocodingApiUrl( $address ) {
			...
			if ( ! empty( $this->settings->geocodingApiKey ) ) {
				$urlParameters['key'] = rawurlencode( $this->settings->geocodingApiKey );
			}

			$urlParameters = apply_filters( 'bgmp_geocoding-api-url-parameters', $urlParameters );
			$url           = add_query_arg( $urlParameters, $url );

			// todo test general stuff, valid/invalid input, using filter, etc
		...
		public function listShortcode( $attributes ) {
			...
			if ( $posts ) {
				$output = '<div id="' . self::PREFIX . 'list" class="' . self::PREFIX . 'list">';    // Note: id should be removed and everything switched to class, because there could be more than one list on a page. That would be backwards-compatability, though.
		...
		/**
		 * Mark a function, hook, etc as being deprecated
		 *
		 * This mimics Core's internal _deprecated_function() function, which isn't used because it's not
		 * guaranteed to be backwards compatible.
		 *
		 * @todo convert all the @deprecated comments everywhere to use this instead
		 *
		 * @param string $symbol
		 * @param string $replacement
		 * @param string $version
		 */
		public function deprecated( $symbol, $replacement, $version ) {
			if ( ! WP_DEBUG ) {
				return;
			}

			trigger_error( sprintf(
				esc_html__( '`%1$s` was deprecated in version %2$s, please use `%3$s` instead.', 'basic-google-maps-placemarks' ),
				$symbol,
				$version,
				$replacement
			) );
		...
		protected function describe( $data, $output = 'die', $message = '' ) {
			...
			$description = sprintf( '
				...
				__( 'Type', 'basic-google-maps-placemarks' ),
				$type,
				__( 'Length', 'basic-google-maps-placemarks' ),
				$length,
				__( 'Content', 'basic-google-maps-placemarks' ),
